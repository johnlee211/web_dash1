...
mode = st.sidebar.radio(
    "화면 선택",
    ["패치노트", "데이터 개요", "단일 작품 모니터링", "두 웹툰 비교", "베스트 댓글 원문"],
)
...
if mode == "패치노트":
    ...
    st.markdown(
        ...
        - **종료/완결 작품 처리**: 요일별 목록에서 사라진 작품은 최신 매핑에서 제외되므로 이후 다운로드 수 수집 대상에서도 제외됩니다. 별도의 제거 목록 저장 기능은 아직 없습니다.
        """
    )
    ...
    st.markdown(
        ...
        - **단일 작품 모니터링**: 회차 흐름, 공개 후 누적 댓글, 작성자 팬층, 팬덤의존도, 다운로드 수
        - **두 웹툰 비교**: 회차별 지표 비교, 구간 병치, 구간 평균, 다운로드 수 비교, 베스트 댓글, 작성자 팬층
        - **베스트 댓글 원문**: 베스트 댓글 원문 필터링 및 확인
        """
    )
...
# ════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(["회차 흐름", "구간 요약", "구간 병치 라인차트", "공개 후 누적 댓글", "전체 지표 통합", "작성자 팬층", "팬덤의존도", "다운로드 수", "상세 테이블"])
    with tab1:

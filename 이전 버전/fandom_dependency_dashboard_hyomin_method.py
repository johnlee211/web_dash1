"""
웹툰 팬덤의존도 대시보드 - 효민 노트북 방식 반영판

실행:
    streamlit run fandom_dependency_dashboard_hyomin_method.py
...
def classify_comment_flags(c: pd.DataFrame) -> pd.DataFrame:
    ...
    # 노트북 기준의 핵심 라벨: 유료결제 댓글 / 무료공개 이후 댓글.
...
def build_reference_time(c: pd.DataFrame, mode: str, notebook_hour: int, fallback_hour: int) -> pd.Series:
    ...
    if mode == "효민 노트북 방식(uploaded_at_dt 날짜 + 지정시각)":
        # 노트북의 uploaded_at_dt + 23h를 일반화. 날짜 컬럼이 00시일 때 정확히 같은 결과입니다.
        ref = uploaded.dt.normalize() + pd.to_timedelta(notebook_hour, unit="h")
        fallback = free_release.fillna(uploaded)
        return ref.fillna(fallback)

    # 검산용: 수집기에 들어있는 free_release_at을 직접 기준으로 쓰는 방식
    fallback = uploaded.dt.normalize() + pd.to_timedelta(fallback_hour, unit="h")
    return free_release.fillna(fallback)


def calculate_fandom_metrics_hyomin(
    ...
    c["reference_at"] = build_reference_time(c, reference_mode, notebook_hour, fallback_hour)
    ...
    # 효민 노트북과 동일한 부등호 구조: 하한선 없이 <= after_3 / <= after_72만 사용.
...
def plot_fandom_dependency(df: pd.DataFrame, title: str, red_top_pct: int, blue_bottom_pct: int, chart_height: int):
    ...
    st.caption(
        ...
        f"상위 {red_top_pct}% ≥ {red_cutoff:.3f} 빨강, 나머지 회색. 효민 노트북의 q25/q75 방식은 25%/25%입니다."
    )
...
st.caption("효민 노트북의 계산 방식으로 사전호응률, 초기호응률, 최종호응률, 팬덤의존도, 결제언급률을 계산합니다.")
...
reference_mode = st.sidebar.radio(
    "기준 공개시각 방식",
    ["효민 노트북 방식(uploaded_at_dt 날짜 + 지정시각)", "수집기 free_release_at 방식(검산용)"],
    index=0,
    help="기본값은 업로드된 노트북과 같은 방식입니다. uploaded_at_dt가 00시로 들어온 것을 지정시각으로 보정합니다.",
)
notebook_hour = st.sidebar.selectbox("효민 방식 기준 시각", list(range(24)), index=23, format_func=lambda x: f"{x:02d}시")
fallback_hour = st.sidebar.selectbox("free_release_at 없을 때 보조 시각", list(range(24)), index=22, format_func=lambda x: f"{x:02d}시")
initial_hours = st.sidebar.number_input("초기호응률 기준 시간", min_value=1.0, max_value=24.0, value=3.0, step=1.0)
base_hours = st.sidebar.number_input("72시간 기준 시간", min_value=6.0, max_value=168.0, value=72.0, step=6.0)
keyword_text = st.sidebar.text_area("결제 키워드", value=PAYMENT_KEYWORDS_DEFAULT, height=80)

st.sidebar.header("팬덤의존도 색상")
red_top_pct = st.sidebar.slider("빨간색: 상위 비율", min_value=5, max_value=40, value=25, step=5)
blue_bottom_pct = st.sidebar.slider("파란색: 하위 비율", min_value=5, max_value=40, value=25, step=5)
...
summary = calculate_fandom_metrics_hyomin(
    comments=comments,
    episodes=episodes,
    reference_mode=reference_mode,
    notebook_hour=notebook_hour,
...
with tab3:
    ...
    st.info("효민 노트북 방식은 72시간 기준 댓글을 actual_written_at <= after_72로 계산합니다. 따라서 유료결제 댓글도 72시간 기준 분모에 포함될 수 있습니다.")
...
with tab4:
    ...
    make_download_button(view[display_cols], f"{title_sel}_팬덤의존도_효민방식.csv")
    ...
    raw["reference_at"] = build_reference_time(raw, reference_mode, notebook_hour, fallback_hour)

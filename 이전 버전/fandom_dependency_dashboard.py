...
def color_dependency(v: float, red_threshold: float, blue_threshold: float) -> str:
    if pd.isna(v):
        return "#CFCFCF"
    if v >= red_threshold:
        return "#D62728"  # red
    if v <= blue_threshold:
        return "#1F77B4"  # blue
    return "#CFCFCF"      # neutral gray


def plot_fandom_dependency(df: pd.DataFrame, title: str, red_threshold: float, blue_threshold: float, height: int):
    data = df.sort_values("episode_no").copy()
    colors = [color_dependency(v, red_threshold, blue_threshold) for v in data["fandom_dependency"]]
...
with tab1:
    ...
    plot_fandom_dependency(view, title_sel, red_threshold, blue_threshold, chart_height)

...
def ws_build_download_event_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tbl = df[cols].rename(columns=keep)
    ...
    if tbl.empty:
        ...
        cols2 = [c for c in keep_no_base if c in df.columns]
        tbl = df[cols2].rename(columns=keep_no_base)
...
def ws_get_download_data(upload_key: str = "download_file") -> pd.DataFrame:
    file_path = ws_find_download_file()
    if file_path:
        raw = ws_load_download_data_from_path(str(file_path))
        return ws_standardize_download_df(raw)
    uploaded = st.file_uploader("series_download_increase_report_history 파일 업로드", type=["csv", "xlsx", "xls"], key=upload_key)
    if uploaded is None:
        return pd.DataFrame()
...
def ws_show_download_section(title_sel: str, meta_df: Optional[pd.DataFrame] = None, key_prefix: str = "download"):
    st.markdown("#### 시리즈 다운로드 수")
    download_df = ws_get_download_data(upload_key=f"{key_prefix}_upload")
    if download_df.empty:
        st.info("`series_download_increase_report_history.csv` 파일을 대시보드 파일과 같은 폴더 또는 data 폴더에 두면 자동으로 읽습니다.")
        return
...
with st.sidebar.expander("파일 업로드, 자동 감지가 안 될 때만 사용", expanded=False):
    comment_uploads = st.file_uploader("댓글 파일 업로드", type=["csv", "xlsx", "xls"], accept_multiple_files=True)
    episode_uploads = st.file_uploader("회차정보 파일 업로드", type=["csv", "xlsx", "xls"], accept_multiple_files=True)
    meta_uploads = st.file_uploader("웹툰 메타/태그 파일 업로드", type=["csv", "xlsx", "xls"], accept_multiple_files=True)
...
if comments_all.empty and episodes_all.empty:
    st.title("웹툰 반응 비교 대시보드")
    st.info(
        "데이터 파일을 찾지 못했습니다. 이 app.py와 같은 폴더 또는 하위 폴더에 댓글/회차정보 CSV, XLSX 파일을 두거나, 사이드바에서 직접 업로드해주세요."
    )
    st.stop()

summary_all = build_episode_summary(comments_all, episodes_all)
webtoon_meta = build_webtoon_meta(comments_all, episodes_all, meta_raw)

if webtoon_meta.empty:
...
mode = st.sidebar.radio(
    "화면 선택",
    ["데이터 개요", "단일 작품 모니터링", "두 웹툰 비교", "시리즈 다운로드", "베스트 댓글 원문", "데이터 확인"],
)
...
if mode == "데이터 개요":
    ...
    with st.expander("태그/장르 정보를 추가하는 방법", expanded=False):
        st.markdown(
            """
            대시보드는 `[웹툰스테이션] 네이버 웹툰 순위 표.xlsx` 같은 메타 파일에서 `웹툰 ID`, `웹툰명`, `해시태그`, `장르` 컬럼을 자동으로 읽습니다.  
            직접 태그를 추가하려면 같은 폴더에 `webtoon_tags.csv` 또는 엑셀 파일을 만들고 아래 컬럼을 넣으면 됩니다.
            """
        )
        st.code("""title_id,title_name,genre,tags
827323,남주의 정석,로맨스,"로맨스,캠퍼스,청춘로맨스"
822657,환생천마,무협,"무협,환생,먼치킨"""
        )
        st.caption("`tags`에는 쉼표, 공백, # 기호가 섞여 있어도 자동 분리합니다. `title_id`가 있으면 가장 안정적으로 연결됩니다.")
    ...
    with st.expander("자동 감지된 파일"):
        st.write("댓글 파일")
        st.code("\n".join(comment_files) if comment_files else "없음")
        st.write("회차정보 파일")
        st.code("\n".join(episode_files) if episode_files else "없음")
...
# ════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(["회차 흐름", "구간 요약", "구간 병치 라인차트", "공개 후 누적 댓글", "전체 지표 통합", "작성자 팬층", "팬덤의존도", "시리즈 다운로드", "상세 테이블"])
    with tab1:
...
# ════════════════════════════════════════════════════════════
    ...
    st.divider()
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["회차별 비교", "구간 병치 라인차트", "구간 평균 비교", "베스트 댓글", "작성자 팬층", "상세 테이블"])
    with tab1:
...
# ════════════════════════════════════════════════════════════
    ...
    st.title("데이터 확인")
    tab1, tab2, tab3 = st.tabs(["댓글 데이터", "회차정보 데이터", "분석용 회차 요약"])
    with tab1:
        st.write(comments_all.shape)
        st.dataframe(comments_all.head(1000), use_container_width=True)
        st.download_button("댓글 데이터 CSV 다운로드", comments_all.to_csv(index=False, encoding="utf-8-sig"), file_name="comments_all_standardized.csv", mime="text/csv")
    with tab2:
        st.write(episodes_all.shape)
        st.dataframe(episodes_all.head(1000), use_container_width=True)
        st.download_button("회차정보 CSV 다운로드", episodes_all.to_csv(index=False, encoding="utf-8-sig"), file_name="episodes_all_standardized.csv", mime="text/csv")
    with tab3:
        st.write(summary_all.shape)
        st.dataframe(summary_all.head(1000), use_container_width=True)
        st.download_button("회차 요약 CSV 다운로드", summary_all.to_csv(index=False, encoding="utf-8-sig"), file_name="episode_summary_all.csv", mime="text/csv")

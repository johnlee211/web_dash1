...
실행
- streamlit run series_download_simple_dashboard.py

기능
- 웹툰 1개를 선택해 수집 시점별 누적 다운로드수와 이전 수집 대비 증가수만 확인합니다.
- y축 2개를 사용합니다.
  - 왼쪽 y축: 누적 다운로드수
  - 오른쪽 y축: 증가수
- 표에는 수집시각, 누적 다운로드수, 이전 수집 대비 증가수만 표시합니다.
"""
...
st.caption("웹툰 1개를 선택해 수집 시점별 누적 다운로드수와 이전 수집 대비 증가수만 확인합니다.")
...
m2.metric("최근 증가수", f"{latest['collect_increase']:,.0f}")
m3.metric("수집 횟수", f"{len(chart_df):,}")

out = chart_df[["collect_label", "series_download_count", "collect_increase"]].copy()
out = out.rename(
    columns={
        "collect_label": "수집시각",
...
st.download_button(
    ...
    file_name=f"{clean_filename(selected_title)}_다운로드추이.csv",
    mime="text/csv",
)
